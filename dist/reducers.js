...
var usersReducer = function (state, action) {
    ...
    if (action.type === 'SET_USERS') {
        return {
            ids: action.payload.users.map(function (user) { return user.id; }),
            all: action.payload.users.reduce(function (acc, user) {
                acc[user.id] = {
                    id: user.id,
...
var projectsReducer = function (state, action) {
    if (state === void 0) { state = initialProjectsState; }
    if (action.type === 'SET_PROJECTS') {
        return {
            selectedId: 1,
            ids: action.payload.map(function (p) { return p.id; }),
            all: action.payload.reduce(function (acc, p) {
                acc[p.id] = {
                    id: p.id,
                    title: p.title,
...
var tasksReducer = function (state, action) {
    if (state === void 0) { state = initialTasksState; }
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, axios_1["default"].get('http://www.mocky.io/v2/5cef982f30000028383cd155')];
                case 1:
                    response = _a.sent();
                    return [3 /*break*/, 3];
                case 2:
